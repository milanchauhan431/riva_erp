				...
				<div class="card">
								...
								<div class="input-group">
									<div class="input-group-append" style="width: 20%;">
										<select name="report_type" id="report_type" class="form-control select2">
											<?php
												foreach($gstr2Types as $key=>$value):
					...
					<div class="card-body reportDiv" style="min-height:75vh">
						<div class="table-responsive">
							<table id='gstr2Table' class="table table-bordered">
...
function loadData(file_type = "VIEW") {
	...
	if (valid) {
		if (file_type == "VIEW") {
			$.ajax({
				url: base_url + controller + '/getGstr2Report',
				data: postData,
				type: "POST",
				dataType: 'json',
				success: function(data) {	
					$("#gstr2Table").html("");
					$("#gstr2Table").html(data.html);
				}
			});
		}else{
			var url = base_url + controller + '/getGstr2Report/' + encodeURIComponent(window.btoa(JSON.stringify(postData)));
			window.open(url);
		}
	}
}
